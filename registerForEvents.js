    ...
    if(validate){
        ...
        database.ref('Users/'+flairId).once('value').then(function(snapshot){
            ...
            if(x != null){
                if(x.pin === pin){
                    if(eventType == "solo")
                        afterAuthenticationforSolo(collection,eventName,eventParent,flairId,x.firstname,x.lastname,x.college_id);
                    else if(eventType == "team")
                        afterAuthenticationforTeam(collection,eventName,eventParent,flairId,x.firstname,x.lastname,x.college_id,teamName,teamNumber);
                    else {
...
//Helper Functions
function afterAuthenticationforSolo(collection,eventName,eventParent,flairId,firstname,lastname,college_id){
    var database = firebase.database();
    database.ref('Events/'+eventParent+'/'+eventName+'/'+flairId).once('value').then(function(snapshot){
        ...
        if(x == null){
            ...
            clearTextfield(collection);
        }else{
            window.alert('You Have already Registered');
        }
    });
}
function afterAuthenticationforTeam(collection,eventName,eventParent,flairId,firstname,lastname,college_id,teamName,teamNumber){
    var database = firebase.database();
    database.ref('Events/'+eventParent+'/'+eventName+'/'+flairId).once('value').then(function(snapshot){
        var x = snapshot.val();
        if(x == null){
...
function clearTextfield(collection){
